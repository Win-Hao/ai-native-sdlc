#!/bin/bash
# Self-test for the StageKit hook scripts. Builds a throwaway repo, drives every
...
unset STAGEKIT_RELEASE_APPROVAL
cd "$T" || exit 1
git init -q && git config user.email t@t && git config user.name t
mkdir -p src tests .github/workflows .stagekit stagekit/0001-x evals
...
echo "== not adopted: no .stagekit/config.json → every hook is silent"
touch .stagekit/lock-tests
...
rm .stagekit/lock-tests
cp "$P/presets/generic.json" .stagekit/config.json
...
rm .stagekit/lock-tests
jq '.drive="suggest"' .stagekit/config.json > t.json && mv t.json .stagekit/config.json
...
jq '.drive="auto"' .stagekit/config.json > t.json && mv t.json .stagekit/config.json
...
contains "quoted status" "$(context prompt-context)" "Current stage: spec. Next: /stagekit:spec"
...
contains "build next" "$(context prompt-context)" "Current stage: build. Next: /stagekit:build"
art plan.md 'status: implemented'
C=$(context prompt-context); contains "review next" "$C" "Current stage: review"; contains "verify rides along" "$C" "/stagekit:verify then /stagekit:review"
art findings.md 'status: reviewed'
C=$(context prompt-context); contains "gates met" "$C" "All artifact gates met"; contains "done offered" "$C" "/stagekit:done"
rm stagekit/0001-x/findings.md

echo "== stop hook: speaks only when the state changes"
rm -f .stagekit/.laststate
...
STAGEKIT_RELEASE_APPROVAL=someone; export STAGEKIT_RELEASE_APPROVAL
...
printf '# Plan\n\n## Files that change\n- `src/b.js` (modify)\n' > stagekit/0001-x/plan.md
echo y >> src/a.js; git add src/a.js
decision "unplanned file asks"       guard-commit '{"command":"git commit -m x"}' ask
contains "message points at /stagekit:done" "$(hook guard-commit '{"command":"git commit -m x"}')" "/stagekit:done"
...
jq --arg f "$T/fmt.sh" '.format_command=$f' .stagekit/config.json > t.json && mv t.json .stagekit/config.json
hook format '{"file_path":"'"$T"'/src/a.js"}' >/dev/null
hook format '{"file_path":"'"$T"'/stagekit/0001-x/intent.md"}' >/dev/null
...
jq '.gates.test_lock.test_globs=["**/nothing_*"]' .stagekit/config.json > "$T/dead.json"
