...
if [ -z "$ID" ]; then
  ...
  jq -n --arg c "[stagekit] No active change. Anything beyond a trivial edit starts with $FIRST." \
...
[ -f "$ROOT/.stagekit/lock-tests" ] && GUARD="$GUARD Test files are LOCKED."

jq -n --arg c "[stagekit] change=$ID $PS_STATE. $GUARD" \
  '{hookSpecificOutput:{hookEventName:"UserPromptSubmit",additionalContext:$c}}'
