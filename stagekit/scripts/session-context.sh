#!/bin/bash
# Frame the session according to .stagekit/config.json → drive:
...
if [ -z "$ID" ]; then
  STATE="## State
No active change (\`.stagekit/current\` is empty). \`$(pipeline_first_cmd)\` opens one."
else
  pipeline_state "$ROOT/$DIR/$ID"
  LOCK=""
  [ -f "$ROOT/.stagekit/lock-tests" ] && LOCK="
...
if [ "$DRIVE" = suggest ]; then
  RULES="# StageKit toolbox (drive: suggest)

Change artifacts live in \`$DIR/<id>/\`; this project's stages are
$CHAIN; the active change id is in \`.stagekit/current\`. The stage commands
(/stagekit:intent /stagekit:spec /stagekit:plan /stagekit:build /stagekit:verify /stagekit:review
/stagekit:done) are the user's to run.
...
id is in \`.stagekit/current\`, and **this project's pipeline is**:

  $CHAIN

The pipeline is defined in \`.stagekit/config.json\` → \`pipeline\`. It is this
...
- A change stays open until the user runs \`/stagekit:done\`. Do not run it for them."
fi
