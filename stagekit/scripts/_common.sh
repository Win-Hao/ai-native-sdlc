#!/bin/bash
# Shared helpers for StageKit gates. Every gate is a no-op unless the project
# has been initialized with /stagekit:init (i.e. .stagekit/config.json exists).
#
# Every hook script opens the same way:
#   IN=$(cat); source _common.sh; CWD=$(hook_cwd); ROOT=$(stagekit_root "$CWD") || exit 0; need_jq gate|context
# Neither the cwd nor the root lookup needs jq, so a repo that has not adopted
# StageKit is never affected by a missing jq.

have_jq() { command -v jq >/dev/null 2>&1; }

# cwd from the hook payload. Without jq it is empty and stagekit_root falls back
...
need_jq() {  # $1 = gate | context
  have_jq && return 0
  local m="[stagekit] jq is not installed, so the StageKit hooks cannot run. The gates fail closed: every Edit, Write and Bash call in this repo is blocked until jq is on PATH (macOS: brew install jq — Debian/Ubuntu: apt install jq). Tell the user now."
...
stagekit_root() {
  ...
  [ -z "$d" ] && d="$PWD"
  while [ "$d" != "/" ] && [ -n "$d" ]; do
    [ -f "$d/.stagekit/config.json" ] && { echo "$d"; return 0; }
    d="$(dirname "$d")"
  done
  return 1
}

cfg() { jq -r "$1 // empty" "$ROOT/.stagekit/config.json" 2>/dev/null; }

gate_on() { [ "$(cfg ".gates.$1.enabled")" = "true" ]; }

# .stagekit/config.json → drive: off (default) | suggest | auto. The gates never
...
current_id() { [ -f "$ROOT/.stagekit/current" ] && tr -d '[:space:]' < "$ROOT/.stagekit/current"; }

# --- pipeline resolution ------------------------------------------------
# Reads .stagekit/config.json .pipeline (falls back to the built-in six stages).
...
DEFAULT_PIPELINE='[{"id":"intent","artifact":"intent.md","done_when":"accepted","command":"/stagekit:intent","draft_hint":"Review it — set status: accepted to open the next stage."},{"id":"spec","artifact":"spec.md","done_when":"accepted","command":"/stagekit:spec","draft_hint":"Resolve its flagged concerns, then set status: accepted."},{"id":"plan","artifact":"plan.md","done_when":"approved","command":"/stagekit:plan","draft_hint":"Interrogate it, then set status: approved."},{"id":"build","artifact":"plan.md","done_when":"implemented","command":"/stagekit:build","draft_hint":null},{"id":"verify","artifact":null,"command":"/stagekit:verify"},{"id":"review","artifact":"findings.md","done_when":"reviewed","command":"/stagekit:review","draft_hint":null}]'
...
pipeline_rows() {
  jq -r --argjson d "$DEFAULT_PIPELINE" --arg us "$US" \
    ...
    "$ROOT/.stagekit/config.json" 2>/dev/null
}
...
pipeline_state() {  # $1 = absolute artifact dir of the active change
  ...
  if [ "$resolved" = 0 ]; then
    PS_STAGE="done"
    PS_NEXT="${pending:+$pending, then }/stagekit:done to close the change"
  fi
  PS_STATE="${PS_STATE% }"
}
