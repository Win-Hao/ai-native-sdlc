...
if gate_on test_lock && [ -f "$ROOT/.stagekit/lock-tests" ]; then
...
lock with: rm .stagekit/lock-tests"
    fi
...
if gate_on protected_paths; then
  while IFS= read -r g; do
    [ -z "$g" ] && continue
    if match_glob "$REL" "$g"; then
      deny "$REL is a protected path (.stagekit/config.json → gates.protected_paths).
