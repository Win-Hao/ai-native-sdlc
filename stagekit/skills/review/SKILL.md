---
name: review
description: Stage 5 Deploy (a) — run the REVIEW.md passes over the change (bugs, security, compliance against spec.md and plan.md), rank findings by severity, tag each with a class, count repeats across earlier reviews, record the result as stagekit/<id>/findings.md, and turn repeats into a CLAUDE.md line, a hook or a skill.
when_to_use: When the user runs /stagekit:review or asks for a review of a diff, branch or PR, or when this repo's StageKit driver is on (`drive: auto`) and a change is verified.
...
2. **Run the passes in `REVIEW.md`.** If it is missing, write it first from
   `.stagekit/templates/REVIEW.md` if the project has one, otherwise `${CLAUDE_PLUGIN_ROOT}/templates/REVIEW.md` — an unwritten review policy is
   ...
   For a large or high-risk diff, run the `stagekit-reviewer` subagent so the passes
   get a clean context.
...
6. **Count repeats.** For each class, look through earlier records:
   `grep -l "| <class> |" stagekit/*/findings.md stagekit/_reviews/*.md`, excluding this
   ...
   - **3rd or more**: the `CLAUDE.md` line is not working. Propose the next
     control and say why: a hook through `/stagekit:gate add` when the mistake is
...
7. **Record it.** Write `stagekit/<id>/findings.md` from `.stagekit/templates/findings.md`
   ...
   `stagekit/_reviews/<YYYY-MM-DD>-<slug>.md` so the count still works. Commit:
...
Usually next: the human merges, then `/stagekit:done` closes the change.
`/stagekit:gate` if the release path is not gated yet.
