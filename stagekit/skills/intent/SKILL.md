...
when_to_use: When the user asks to capture, write up or start an intent (or runs /stagekit:intent), when an alert, ticket or scan finding needs writing up as intent.md, or when this repo's StageKit driver is on (`drive: auto` in .stagekit/config.json) and new work arrives.
...
belongs in `/stagekit:spec`, and mixing them is what makes intent unreviewable.
...
2. **Check the history.** Search `stagekit/*/intent.md` for earlier changes on the
...
4. **Allocate an id.** `<NNNN>-<slug>`, where NNNN is the next number in `stagekit/`
   (zero-padded to 4). Create `stagekit/<id>/`.

5. **Write `intent.md`** from `.stagekit/templates/intent.md` if the project has one, otherwise `${CLAUDE_PLUGIN_ROOT}/templates/intent.md`.
...
7. **Set the active change**: write the id to `.stagekit/current`.

8. **Commit** `stagekit/<id>/intent.md` and `.stagekit/current`. Author and timestamp
...
stays in `stagekit/` with its reason under a `## Rejected` heading — date, who,
why — so the same ask is recognised next time.

Usually next: `/stagekit:spec`, or `/stagekit:plan` in a flow without a spec stage.
