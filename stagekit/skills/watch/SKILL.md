...
3. **Define the tiers** in `bands.yaml` from `.stagekit/templates/bands.yaml` if the project has one, otherwise `${CLAUDE_PLUGIN_ROOT}/templates/bands.yaml`:
...
Write it to `stagekit/<id>/intent.md` with `origin: alert`, and stop. From there it
...
`## Rejected` heading; it stays in `stagekit/` so the next occurrence is recognised
(`/stagekit:intent` checks prior rejections before opening a change). When a fix
ships, add an eval for the incident (`/stagekit:evals add`) so the class is covered
from then on.
