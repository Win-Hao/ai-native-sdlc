---
name: verify
description: Stage 4 Test — verify the current change before a human sees it. Runs build, test and lint, checks every item of plan.md's Proof, and gets an independent verdict from the stagekit-verifier subagent.
when_to_use: When the user runs /stagekit:verify or asks to verify a change before review, or when this repo's StageKit driver is on (`drive: auto`) and a change is implemented.
...
`/stagekit:fix` (a bug, test first) and `/stagekit:build tdd` (a feature, one slice at
a time).
...
1. **Run build, test and lint** — the commands in `CLAUDE.md`. Paste the literal
   ...
   `/stagekit:init`, which sets one up. A check you could not run is not a pass.
...
4. **For anything non-trivial, run the `stagekit-verifier` subagent.** A fresh
