...
when_to_use: When the user asks for a plan or runs /stagekit:plan, when they ask to implement something that spans more than a few files and no plan.md exists (offer it), or when this repo's StageKit driver is on (`drive: auto`).
...
still a matter of editing a document. Produce `stagekit/<id>/plan.md`.
...
intentions. It also means **you cannot write `stagekit/<id>/plan.md` while in plan
...
4. Only after acceptance: write `stagekit/<id>/plan.md` and commit it (step 5).
...
2. **Draft the plan** in the shape of `.stagekit/templates/plan.md` if the project has one, otherwise `${CLAUDE_PLUGIN_ROOT}/templates/plan.md`:
...
3. **Interrogate the plan before showing it as done.** Ask yourself, with
   ...
   For anything non-trivial, run the `stagekit-plan-critic` subagent over the draft
...
4. **Present it through `ExitPlanMode`** — name the riskiest step out loud —
   ...
   recommended answer — the same format as `/stagekit:intent`. Facts you can
   look up, you look up. A plan with an open decision is not ready to approve.

5. **After acceptance, write and commit.** Create `stagekit/<id>/plan.md` from the
...
Usually next: `/stagekit:build`, or `/stagekit:build tdd` to go red-green one slice at a time.
