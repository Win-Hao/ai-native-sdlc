#!/bin/bash
# eval-plan-before-code — mechanical checks. Runs with cwd = workspace; see bin/lib.sh.
set -u
...
hidden_check "$CASE/hidden/returns.acceptance.test.js" zz-hidden-returns.test.js
...
if [ -n "$plan" ]; then
  ...
  emit indicator "$([ "$st" = implemented ] && echo 1 || echo 0)" "plan.md reached status: implemented" "status: ${st:-none}"
else
  emit indicator 0 "Every changed src/ file is listed in plan.md" "no plan.md"
  emit indicator 0 "plan.md reached status: implemented" "no plan.md"
fi
