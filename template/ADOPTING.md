...
cd <你的项目> && claude
/stagekit:init          # 模板语言、闸门级别、drive（默认 off）、CLAUDE.md 指针块；写之前预览
/stagekit:intent        # 开始第一个变更
```

前提：`jq` 在 PATH 上（macOS 15+ 自带；否则 `brew install jq` / `apt install jq`）。
缺了它闸门会拒绝一切编辑并提示安装，不会静默放行。

`/stagekit:init` 之后 `/stagekit:init doctor` 会告诉你哪些闸门是空的（匹配不到任何文件的 glob
...
```bash
cp -r template/. /path/to/your-repo/    # 注意是 `.` 不是 `*`，要带上 .stagekit/ .github/
```
...
| `.stagekit/config.json` | 闸门配置（generic preset，需要按仓库改） |
| `.stagekit/templates/` | **中文占位版**产物模板（intent / spec / plan / CLAUDE.md / bands.yaml）。插件本身是英文的，有这个目录就优先用中文版；要英文就删掉它 |
| `stagekit/0000-example-claims-status/` | 完整走通的产物链范例：intent → spec → plan |
...
3. **`intent.md`** — `/stagekit:intent`，哪怕只有你一个人

这三件做完，agent 的输出质量已经变了，后面的都是在这个基础上加约束。

### 第 2–4 周——升到 `standard`
4. **plan mode 成为默认起手式** — `/stagekit:plan`，任何非平凡改动先出 plan.md
5. **打开 test_lock + protected_paths 闸门** — 升到级别 `standard`，`/stagekit:gate audit` 看现状
6. **`REVIEW.md` + `/stagekit:review`** — 三遍 passes

### 第 2 个月起——闸门 `full`，`drive: suggest` 或 `auto`
7. **skills** — 每条反复要求却总被忘记的规范写一个
8. **hooks 作为审批闸门** — `/stagekit:gate add`，把组织自己的审批（变更单、迁移）表达成闸门；`plan_sync` 也在这一步开
9. **evals** — 攒够 20 个真实任务再进 CI，不然门槛是噪音
10. **`/stagekit:watch`** — 挑一个指标闭环，rollback 路径先演练过再开 3σ
...
**2. 闸门配置照抄不校验。**
`.stagekit/config.json` 的 preset 是猜的。你的测试目录可能不叫 `tests/`，你的生成代码可能
不在 `generated/`。跑 `/stagekit:init doctor`，把 WARN 清干净。

**3. 把 intent 和 spec 混着写。**
`/stagekit:intent` 阶段不讨论实现是硬规则。一旦在 intent 里写了"用 Redis 缓存"，后面
