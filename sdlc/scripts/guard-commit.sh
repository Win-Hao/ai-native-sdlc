...
ID=$(current_id); [ -z "$ID" ] && exit 0   # no open change — see /sdlc:done
...
while IFS= read -r f; do
  [ -z "$f" ] && continue
  case "$f" in "$DIR"/*|.sdlc/*) continue;; esac
  skip=0
...
(If change $ID has shipped, close it with /sdlc:done and this check stops.)"
