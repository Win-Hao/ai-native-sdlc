#!/bin/bash
# Check that .sdlc/config.json actually matches this repo.
# A preset that matches nothing is a gate that protects nothing.
# Usage: doctor.sh [repo-root] [config-file]
#   config-file defaults to <repo-root>/.sdlc/config.json. Pass a candidate
#   file to preview a configuration before it goes live.
ROOT="${1:-${CLAUDE_PROJECT_DIR:-$PWD}}"
command -v jq >/dev/null 2>&1 || { echo "FAIL  jq is not installed — every SDLC hook and this doctor need it (macOS: brew install jq — Debian/Ubuntu: apt install jq)"; exit 1; }
LIVE="$ROOT/.sdlc/config.json"
CFG="${2:-$LIVE}"
[ -f "$CFG" ] || { echo "FAIL  no $CFG — run /sdlc:init"; exit 1; }
...
[ "$CFG" != "$LIVE" ] && echo "== PREVIEW of $CFG — nothing is enforced until it is at .sdlc/config.json"
...
if enabled test_lock; then
  ...
  say "off" "test_lock is disabled — /sdlc:gate raises the level"
fi
...
if enabled protected_paths; then
  ...
  say "off" "protected_paths is disabled — /sdlc:gate raises the level"
fi
