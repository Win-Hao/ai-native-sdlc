---
name: customize
description: Adapt the SDLC to this project — change the stage pipeline (add, remove or reorder stages), override the artifact templates, and point stages at project-specific skills. Use when the default six stages do not fit how this project actually works.
argument-hint: "[pipeline | templates | show]"
disable-model-invocation: true
allowed-tools: Read Write Edit Glob Grep Bash
---

# Adapt the SDLC to this project

Three things are project-owned. Everything else comes from the plugin.

| What | Lives in | Overrides |
|---|---|---|
| The stage pipeline | `.sdlc/config.json` → `pipeline` | the built-in six stages |
| Artifact templates | `.sdlc/templates/*.md` | `${CLAUDE_PLUGIN_ROOT}/templates/` |
...
The pipeline is what `/sdlc:status` and the driver (`drive: suggest | auto`)
follow; with `drive: off` it only orders the status report. It is an ordered
list in `.sdlc/config.json`. Each entry:
...
- **Several stages may share one artifact.** Their `done_when` values become that
  ...
  `implemented`, so `/sdlc:plan` and `/sdlc:build` are two stages on one file.
...
| `${CLAUDE_PLUGIN_ROOT}/pipelines/tdd.json` | build runs as `/sdlc:build tdd` | red-green on features, one slice at a time |

Install one with:
```
jq --slurpfile p <preset> '.pipeline=$p[0]' .sdlc/config.json > t && mv t .sdlc/config.json
```

### Adding a custom stage

1. Write the stage as a **project skill** at `.claude/skills/<name>/SKILL.md`. It
   should end by writing `sdlc/<id>/<artifact>` with a `status` frontmatter field.
2. Add a pipeline entry pointing `command` at `/<name>`.
3. Add a template at `.sdlc/templates/<artifact>` so the shape is consistent.
...
cp ${CLAUDE_PLUGIN_ROOT}/templates/intent.md .sdlc/templates/
```

Every skill reads `.sdlc/templates/<name>` first and falls back to the plugin's.
...
- **Gates** (`.sdlc/config.json` → `gates`) — use `/sdlc:gate`.
