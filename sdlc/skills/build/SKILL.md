...
when_to_use: When the user asks to implement an approved plan.md or runs /sdlc:build, or when this repo's SDLC driver is on (`drive: auto`) and plan.md is approved.
...
`sdlc/<id>/plan.md`. If it is still `draft`, say so and ask — the user may
...
build from: offer `/sdlc:plan`, or do the work as ordinary implementation if
the user prefers.
...
2. **`tdd` — one slice at a time.** When the user passes `tdd` (or the pipeline
   runs build as `/sdlc:build tdd`), every slice in Order of work goes red
   ...
   "tests pass", "it works" — stop and go back to `/sdlc:plan`. Per slice:
   ...
   them alone, `test(<id>): red — <slice>` → `touch .sdlc/lock-tests` →
   implement with the least code that passes, nothing for later slices →
   `rm .sdlc/lock-tests`, saying so → next slice. Writing every test before
...
6. **Before reporting done**, run build + test + lint and paste the literal
   ...
   "build" until it sees it. Then run `/sdlc:verify`.
