...
when_to_use: When the user asks for a spec or runs /sdlc:spec, or when this repo's SDLC driver is on (`drive: auto`) and intent.md has been accepted.
...
time instead of discovered in review weeks later. Produce `sdlc/<id>/spec.md`:
**the decisions a plan can be written from**, not the work itself.

## Inputs

`sdlc/<id>/intent.md` when there is one — the argument names the change, else
`.sdlc/current`. If its status is not `accepted`, say so and ask whether to go
...
id (`<NNNN>-<slug>`, next number in `sdlc/`) and set `.sdlc/current` yourself.
...
3. **Write `spec.md`** from `.sdlc/templates/spec.md` if the project has one, otherwise `${CLAUDE_PLUGIN_ROOT}/templates/spec.md`:
...
file before build starts. A spec with open concerns does not go to `/sdlc:plan`.

When a policy owner is not in the session, offer a questionnaire they can
answer asynchronously: `sdlc/<id>/questions-<owner>.md` — the purpose and the
