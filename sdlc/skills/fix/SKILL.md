...
when_to_use: When the user reports something broken, throwing, failing or slow, asks to fix a bug, debug or diagnose, or runs /sdlc:fix.
...
(`/sdlc:intent` → `/sdlc:plan`) instead of growing it here.

`<id>` in the commit messages below is the active change (`.sdlc/current`) if
...
5. **Lock the tests**: `touch .sdlc/lock-tests`. With the `test_lock` gate on
...
7. **Unlock and clean up**: `rm .sdlc/lock-tests`, and say that you did. Re-run
...
Usually next: `/sdlc:verify`, then `/sdlc:review`.
