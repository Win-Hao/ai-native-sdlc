...
1. Read `.sdlc/config.json` and report the gate level (`_gate_level`), the drive
...
`_gate_level` in `.sdlc/config.json` records how far this repo has gone. A gate on
...
```
jq '._gate_level="standard" | .gates.test_lock.enabled=true | .gates.protected_paths.enabled=true' .sdlc/config.json > t && mv t .sdlc/config.json
jq '._gate_level="full" | .gates.plan_sync.enabled=true' .sdlc/config.json > t && mv t .sdlc/config.json
```
...
3. **Most gates are config, not code.** Prefer adding a pattern to
   `.sdlc/config.json` over writing a new hook script.
